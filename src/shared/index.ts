...
type PeerDIDResolverFN = (did: string) => Promise<SDK.Domain.DIDDocument>;
...
export function didUrlFromString(didString: string) {
    ...
    const did = SDK.Domain.DID.fromString(`did:${method}:${idstring}`);
    const paths = path ? path.split("/").filter((p) => p) : [];
    return new SDK.Domain.DIDUrl(did, paths, attributes, fragment.slice(1));
}
